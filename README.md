...
```
mlops_stack_test        <- Root directory. Both monorepo and polyrepo are supported.
│
├── mlops_stack_test       <- Contains python code, notebooks and ML resources related to one ML project. 
│   │
...
* An automated integration test is provided (in `.github/workflows/mlops_stack_test-run-tests.yml`) that executes a multi task run on Databricks involving the feature engineering and model training notebooks.
...
* Run `databricks configure --profile mlops_stack_test-dev --token --host <your-dev-workspace-url>`, passing the URL of your dev workspace.
...
* From within the root directory of the current project, use the [dbx sync](https://dbx.readthedocs.io/en/latest/guides/python/devloop/mixed/#using-dbx-sync-repo-for-local-to-repo-synchronization) tool to copy code files from your local machine into the Repo by running
  `dbx sync repo --profile mlops_stack_test-dev --source . --dest-repo your-repo-name`, where `your-repo-name` should be the last segment of the full repo name (`/Repos/username/your-repo-name`)
